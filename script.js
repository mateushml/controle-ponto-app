document.addEventListener('DOMContentLoaded', () => {
    ...
    const generateReport = (year, month) => {
        ...
        reportGrid.innerHTML = `
            <div class="report-item"><span>Saldo Final</span><strong style="color: ${monthStats.finalBalance < 0 ? 'var(--danger-color)' : 'var(--success-color)'}">${formatMinutes(monthStats.finalBalance)}</strong></div>
            <div class="report-item"><span>Total Trabalhado</span><strong>${formatMinutes(monthStats.totalWorked, false)}</strong></div>
    ...
    const init = () => {
        ...
        deleteEntryBtn.addEventListener('click', () => {
            ...
            docRef.update({ [fieldToDelete]: firebase.firestore.FieldValue.delete() }).catch(error => console.error("Erro ao apagar registro:", error));
            closeModal();
        });
        ...
        addHolidayBtn.addEventListener('click', () => {
            ...
            const docId = `${year}-${String(month + 1).padStart(2, '0')}`;
            const docRef = db.collection('pontoData').doc(docId);
            docRef.update({ holidays: firebase.firestore.FieldValue.arrayUnion(holidayValue) }).catch(error => { if (error.code === 'not-found') { docRef.set({ holidays: [holidayValue] }, { merge: true }); } else { console.error("Erro ao adicionar feriado:", error); } });
